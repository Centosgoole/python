...
class TongyongSpiderSpider(scrapy.Spider):
    name = 'Poland_spider'
    allowed_domains = ['pol.bizdirlib.com']
    start_urls = ['https://pol.bizdirlib.com/company']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "https://pol.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("https://pol.bizdirlib.com"+next_link,callback=self.parse)
