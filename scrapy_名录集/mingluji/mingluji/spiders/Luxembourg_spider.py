...
class LuxembourgSpiderSpider(scrapy.Spider):
    name = 'Luxembourg_spider'
    allowed_domains = ['lux.bizdirlib.com']
    start_urls = ['http://lux.bizdirlib.com/company?page=140']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "https://lux.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("https://lux.bizdirlib.com"+next_link,callback=self.parse)
