...
class RussiaSpiderSpider(scrapy.Spider):
    name = 'Russia_spider'
    allowed_domains = ['rus.bizdirlib.com']
    start_urls = ['http://rus.bizdirlib.com/company?page=290']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "https://rus.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("https://rus.bizdirlib.com"+next_link,callback=self.parse)
