...
class ItalySpiderSpider(scrapy.Spider):
    name = 'Italy_spider'
    allowed_domains = ['it.bizdirlib.com']
    start_urls = ['https://it.bizdirlib.com/company?page=15592']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "https://it.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("https://it.bizdirlib.com"+next_link,callback=self.parse)
