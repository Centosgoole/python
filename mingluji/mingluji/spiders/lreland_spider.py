...
class LrelandSpiderSpider(scrapy.Spider):
    name = 'lreland_spider'
    allowed_domains = ['irl.bizdirlib.com']
    start_urls = ['http://irl.bizdirlib.com/company?page=1400']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "https://irl.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("https://irl.bizdirlib.com"+next_link,callback=self.parse)
