...
class EstoniaSpiderSpider(scrapy.Spider):
    name = 'Estonia_spider'
    allowed_domains = ['est.bizdirlib.com']
    start_urls = ['http://est.bizdirlib.com/company']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):#获取每个url
            link = "https://est.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
            yield scrapy.Request(link,callback=self.prase1)
        next_link = response.xpath('//*[@id="block-system-main"]/div/div/div[3]/ul/li[3]/a/@href').extract_first()
        if next_link:#获取下一页url
            next_link = next_link
            yield scrapy.Request("https://est.bizdirlib.com"+next_link,callback=self.parse)
