...
class CzechSpiderSpider(scrapy.Spider):
    name = 'czech_spider'
    allowed_domains = ['cze.bizdirlib.com']
    start_urls = ['http://cze.bizdirlib.com/company']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "http://cze.bizdirlib.com/company"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("http://cze.bizdirlib.com/company"+next_link,callback=self.parse)
