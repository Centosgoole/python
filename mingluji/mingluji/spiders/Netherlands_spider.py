...
class NetherlandsSpiderSpider(scrapy.Spider):
    name = 'Netherlands_spider'
    allowed_domains = ['nld.bizdirlib.com']
    start_urls = ['https://nld.bizdirlib.com/company?page=11358']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "https://nld.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("https://nld.bizdirlib.com"+next_link,callback=self.parse)
