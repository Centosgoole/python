...
class LiechtensteinSpiderSpider(scrapy.Spider):
    name = 'Liechtenstein_spider'
    allowed_domains = ['lie.bizdirlib.com']
    start_urls = ['http://lie.bizdirlib.com/company']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "https://lie.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("https://lie.bizdirlib.com"+next_link,callback=self.parse)
