...
class AutriaSpiderSpider(scrapy.Spider):
    name = 'Autria_spider'
    allowed_domains = ['at.bizdirlib.com']
    start_urls = ['https://at.bizdirlib.com/company?page=13888']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "https://at.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("https://at.bizdirlib.com"+next_link,callback=self.parse)
