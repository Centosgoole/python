...
class BelgiumSpiderSpider(scrapy.Spider):
    name = 'Belgium_spider'
    allowed_domains = ['be.bizdirlib.com']
    start_urls = ['https://be.bizdirlib.com/company?page=15193']

    def parse(self, response):#获取url
        url_list = response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li')
        for i in range(len(url_list)):
            link = "https://be.bizdirlib.com"+str(response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/ul/li['+str(i)+']/div/span/a/@href').extract_first())
        ...
        if next_link:
            next_link = next_link
            yield scrapy.Request("https://be.bizdirlib.com"+next_link,callback=self.parse)
